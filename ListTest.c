...
int main() {
  ...
  if (isInList(l,"rabin"))
    puts("rabin is in list");
  else
    puts("rabin is not in list");
  if (isInList(l,"ojha"))
    puts("ojha is in list");
  else
    puts("ojha is not in list");
  int c;
    FILE *file;
    file = fopen("C:\\Users\rabinkumar\Desktop\RabinShotestPath\FRANCE.MAP", "r");
    if (file) {
